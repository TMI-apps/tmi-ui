...
To **control how far upgrades go**, set an explicit range in `package.json` (for example `^0.8.3`) after checking [Releases](https://github.com/TMI-apps/tmi-ui/releases) or [the package on npm](https://www.npmjs.com/package/@tmi-packages/ui).

## 2. CI (GitHub Actions)

For a typical app, **`pnpm install` / `npm ci` need no token** for `@tmi-packages/ui` — the package is public on npm.
...
**If your repository still has** a root `.npmrc` that sets **`@tmi-apps:registry=…`** (legacy GitHub Packages mapping from the old package name) **or** similar overrides, **remove** them once everything you install from npm uses the public registry — otherwise installs may still hit GitHub Packages. See [Consumer setup](./consumer-setup.md#migrating-from-github-packages).

**Other private GitHub Packages** — If you still pull _other_ private packages from `npm.pkg.github.com`, keep only the mappings you need and ensure `NODE_AUTH_TOKEN` is set where those installs run; `@tmi-packages/ui` itself no longer requires `GH_PACKAGES_READ_TOKEN` or `read:packages` for npm-only consumers.

## 3. Local development

No PAT or `~/.npmrc` token is required for `@tmi-packages/ui` when using the public npm registry.
...
```ts
import { ThumbnailPill, VideoEmbedModal } from "@tmi-packages/ui";
import "@tmi-packages/ui"; // if you rely on MUI theme augmentation from this package
```
